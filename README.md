...
### Corresponding to each database the system maintains an oplog (operation log) file that:
...
- When we were using our system's timestamp to microsecond precision, for testing, some of our MERGE operation test cases were failing. On investigating we found that when we ran the same test cases but with our system timestamp to nanosecond precision, all of them passed. The timestamps are assigned to the operations in the oplogs files once the files are parsed. The operations are executed sequentially by the system. The MERGE operation was failing even when there was a gap between 2 UPDATE operations for a (student ID, course ID) tuple for 2 DBs. Thus, ideally when these 2 DBs are MEREGEd then this test case must pass, that is, the DB getting updated must reflect the change based on the test case. Thus, the test cases were failing because the gap between the 2 timestamps was to the microsecond precision. This proves that the system is providing low latency while processing the operations, due to which more than one operation are getting processed in a microsecond making the timestamps for these operations same at the microsecond level. Thus, the system is optimized for low latency performance.
